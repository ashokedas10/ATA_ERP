...
class General_library 
{
	...
	{
				$HttpApiKey='010tt5zw108ydunVvGNpQ3wkYPpwMV';	
				$Uid='718933';
				$route='1'; 
				$MobileNo=	$TextMsg=$SenderId='SATNAM';
				$TextMsg = preg_replace(array('/\s{2,}/', '/[\t\n]/'), ' ', $TextMsg);
						...
						//API URL
						$url="http://smspromo.buzzinfinity.co.in/api/pushsms.php";
						// init the resource
						$ch = curl_init();
	...
	public function getAddress($latitude,$longitude)
	{
		if(!empty($latitude) && !empty($longitude)){
			...
			$geocodeFromLatLong = file_get_contents('http://maps.googleapis.com/maps/api/geocode/json?latlng='.trim($latitude).','.
