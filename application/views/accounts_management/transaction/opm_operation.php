...
<div  ng-app="Accounts">	
<div class="container" style="width:100%"  ng-controller="experimental_form" 
ng-init="view_list(0)" id="myBody" onkeypress = "shortcut()">
				...
				<div class="panel-heading">
					<div class="row">
						<div class="col-sm-12">
							...
							<span class="glyphicon glyphicon-info-sign"></span>OPM Operation {{formname}}</h4>
						</div>
					</div>
				</div>
			</div>	
					...
					<div  ng-if="FormInputArray[0]['header'][header_index]['section_type']== 'GRID_ENTRY'">	
								...
								<!--GRID BODY SECTION-->								
									...
									<table  class="table table-condensed nopadding"  style="overflow:auto">
												...
												<tr  class="bg-primary">
												<td ng-repeat="steps in FormInputArray[0]['header'][header_index]['fields'][0]" 
												ng-init="$index==0"  ng-if="steps.InputType != 'hidden'">	
													{{steps.LabelName}}													</td>
													<td>Save</td>
													<td>Resource</td>
												</tr>
									
												<tr ng-repeat="field_index in 
												[0,FormInputArray[0]['header'][header_index]['fields'].length] | toRange">
													
													<td ng-repeat="steps in 
													FormInputArray[0]['header'][header_index]['fields'][field_index]" 
													...
													<td ng-if="FormInputArray[0]['header'][header_index]['fields'].length!= $index">
													 ...
													 name="Save" ng-click="savedata()">Save</button>
													</td>
													
													<td ng-if="FormInputArray[0]['header'][header_index]['fields'].length!= $index">
													<button type="button" class="btn btn-success" id="Save" 
													name="Save" ng-click="opm_operation_resource(FormInputArray[0]['header'][header_index]['fields'][field_index]['id']['Inputvalue'])">Resource</button>
													</td>
				...
				<div  ng-repeat="header_index in [0,ResourceArray[0]['header'].length] | toRange"   id="maindiv" >	
					...
					<div  ng-if="ResourceArray[0]['header'][header_index]['section_type']== 'GRID_ENTRY'">	
								...
								<!--GRID BODY SECTION-->								
									...
									<tr  class="bg-primary">
									<td ng-repeat="steps in ResourceArray[0]['header'][header_index]['fields'][0]" 
									ng-init="$index==0"  ng-if="steps.InputType != 'hidden'">	
										{{steps.LabelName}}</td>
										<td>Edit</td>
									</tr>
									
									<tr ng-repeat="field_index in [0,ResourceArray[0]['header'][header_index]['fields'].length] 
									| toRange">
													<td ng-repeat="steps in 
													ResourceArray[0]['header'][header_index]['fields'][field_index]" 
													ng-init="Index2 = $index"  ng-if="steps['InputType']!= 'hidden'">	
													 ...
													 <td ng-if="ResourceArray[0]['header'][header_index]['fields'].length!= $index">
													
													<button type="button" class="btn btn-success" id="Save" 
													name="Save" ng-click="opm_operation_savedata()">Save</button>
													</td>
					...
					<tr class="bg-success">
						...
						<!--<button type="submit" class="btn btn-success">Outside Services</button> 
						 <button type="submit" class="btn btn-success">Catalog</button> 
						  ...
						  <a data-toggle="modal" data-target="#search_modal"><button type="submit" 
						  class="btn btn-success" ng-click="main_grid(1)">Search</button></a>
						 ...
						 ng-click="form_data_save()">Submit</button>-->
						 
						 <!-- <a data-toggle="modal" data-target="#shortModal"><button type="submit" 
