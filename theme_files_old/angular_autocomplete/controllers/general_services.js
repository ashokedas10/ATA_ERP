...
// Accounts Sections
 ...
 GeneralServices.factory('PurchaseEntry',['$http','$rootScope',function($http,$rootScope)
 {
	...
	factoryobj.list_items=function(param,trantype){
		...
		if(param=='hq_id_name')
		{	
			var data_link=BaseUrl+"hq_id_name/"+trantype;
			console.log(data_link);
			$http.get(data_link).then(function(response) {$rootScope.list_party=response.data;});	
			angular.forEach($rootScope.list_products, function(value, key) {$rootScope.searchItems.push(value.name);});				
		}	
 ...
 GeneralServices.factory('Sale_test',['$http','$rootScope',function($http,$rootScope)
 {
	...
	factoryobj.list_items=function(param,product_id){
		...
		if(param=='hq_id_name')
		{	
			var data_link=BaseUrl+"hq_id_name/"+product_id;
			console.log(data_link);
			$http.get(data_link).then(function(response) {$rootScope.list_hq=response.data;});	
			angular.forEach($rootScope.list_hq, function(value, key) {$rootScope.searchItems.push(value.name);});	
		...
		if(param=='batchno'){	
			...
			console.log(data_link);			
			$http.get(data_link)
			.then(function(response) {$rootScope.list_batch=response.data	;});			
			angular.forEach($rootScope.list_batch, function(value, key) {$rootScope.searchItems.push(value.name);});						
		}	
		
		if(param=='tbl_party_id_name'){angular.forEach($rootScope.list_party, function(value, key) {$rootScope.searchItems.push(value.name);});}	
